  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Save store slug to local storage
        const slug = storeName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
        localStorage.setItem('storeSlug', slug);
        
        // Redirect to admin panel instantly
        router.push('/admin');
      } else {
        ...
        <div className="text-center mb-12 animate-fadeIn">
          <Link href="/" className="inline-flex items-center gap-3 mb-4 group">
            ...
            <div className="flex flex-col items-start">
               ...
               <span className="text-[10px] font-black text-primary tracking-[0.3em] uppercase">Partner Program</span>
            </div>
          </Link>
        </div>
        ...
        <div className="card-premium p-10 md:p-12 animate-slideUp">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="storeName" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
                Restaurant Name
              </label>
            ...
            {/* Password Input */}
            <div className="space-y-2">
              <label htmlFor="password" className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
                Password
              </label>
              <div className="relative group">
                 <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-primary transition-colors">
            ...
            >
              {isLoading ? (
                <div className="flex items-center gap-3">
                   ...
                   <span className="font-black uppercase tracking-widest text-xs">Creating Free Account...</span>
                </div>
              ) : (
                <>
          ...
          <div className="mt-12 pt-8 border-t border-gray-50 flex items-center justify-between">
             <Link href="/login" className="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-primary transition-colors flex items-center gap-2">
